...
if __name__ == "__main__":
    ...
    for train_indices, test_indices in kfold.split(data, target):
        ...
        for iteration in range(args.iterations):
            ...
            for i in permutation:
                grad = (train_data[i].T @ weights - train_target[i]) * train_data[i]
        ...
        explicit_rmse = np.sqrt(sklearn.metrics.mean_squared_error(pred, test_target))
        print("Test RMSE on fold {}: SGD {:.2f}, explicit {:.2f}".format(len(rmses), rmses[-1][-1]["test"], explicit_rmse))

    if args.plot:
        for i in range(len(rmses)):
            plt.plot(range(args.iterations), [x["train"] for x in rmses[i]], label=f"train-{i}")
            plt.plot(range(args.iterations), [x["test"] for x in rmses[i]], label=f"test-{i}")
