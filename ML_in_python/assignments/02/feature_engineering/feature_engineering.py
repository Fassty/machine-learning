...
if __name__ == "__main__":
    ...
    train, test = train_test_split(dataset["data"], test_size=args.test_ratio, random_state=args.seed)
    ...
    categorical_features = list(range(dataset["data"].shape[1]))
    non_categorical = []
    ...
    non_categorical.sort()

    #ct = ColumnTransformer([("categorical", OneHotEncoder(), categorical_features),("non-categorical", StandardScaler(), non_categorical)])
    ...
    pipeline = Pipeline([
        ('Transformer', ColumnTransformer(
            [
                ...
                ("non-categorical", StandardScaler(), non_categorical)])),
        ("Poly", PolynomialFeatures(2, include_bias=False))])
