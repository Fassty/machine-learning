...
def accuracy(X, y):
    return np.mean(sum([np.argmax(forward(X[i])[1]) == y[i] for i in range(X.shape[0])]))
...
def gradient(w, X, y):
    ...
    for i in range(X.shape[0]):
        h, o = forward(X[i])
        o[y[i]] -= 1
        dh = (w[1].T @ o) * dReLU(w[0] @ X[i])

        grad[0] += np.outer(dh, X[i])
        grad[1] += np.outer(o, h)
...
if __name__ == "__main__":
    ...
    weights = [np.random.uniform(size=[train_data.shape[1], args.hidden_layer], low=-0.1, high=0.1).T,
               np.random.uniform(size=[args.hidden_layer, args.classes], low=-0.1, high=0.1).T]
    ...
    def forward(inputs):
        ...
        h = ReLU(weights[0] @ inputs)
        o = softmax(weights[1] @ h)
        return h,o
