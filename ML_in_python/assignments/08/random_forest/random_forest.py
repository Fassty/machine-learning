...
class DecisionTree:
    ...
    def _best_split(self, node):
        best_criterion = None
        uni = generator.uniform(size=self._data.shape[1]) <= args.feature_subsampling
        features = np.array(list(range(self._data.shape[1])))
        features = features[uni]
...
if __name__ == "__main__":
    ...
    generator = np.random.RandomState(args.seed)
    estimators = np.full(args.n_estimators, None)

    for i in range(args.n_estimators):
        model = DecisionTree(args.max_depth, args.min_to_split)

        if args.bootstrapping:
            indices = generator.randint(len(train_data), size=len(train_data))
            model.fit(train_data[indices], train_target[indices])
        else:
    ...
    def predict(x):
        ...
        for i in range(pred.shape[1]):
            ...
            pr.append(np.argmax(counts))

        return pr

    def accuracy(X, y):
        pred = predict(X)
        return np.mean([1 if pred[i] == y[i] else 0 for i in range(X.shape[0])])
