...
data.train <- s[1:1600]
data.test <- s[1601:2000]
# length(unique(c(data.train,data.test)))	# just to check

# to train the model/hypothesis
m = rpart(profits ~ category + sales + assets + marketvalue, F[data.train, 1:8], cp=0.0066)

forbes.train = F[data.train, 1:8]
forbes.test = F[data.test, 1:8]
prediction = predict(m, forbes.train, type="class")
m.cm = table(prediction, forbes.train$profits)
message("Accuracy train= ", sum(diag(m.cm)) / 1600)

prediction = predict(m, forbes.test, type="class")
m.cm = table(prediction, forbes.test$profits)
message("Accuracy test= ", sum(diag(m.cm))/ 400)
# to see the resulting DT
library(rpart.plot)
rpart.plot(m)    # only its topology
dev.new()
